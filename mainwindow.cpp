...
int  orn = 1;			// ориентация листа 1 - портретная
...
// создание привьюшки
void MainWindow::show_img (QString fl)
{
    prew pr;
    img_count++;
    toshow.resize(img_count+1);
    toshow[img_count].buf=buf;
    QPicture p; // = toprint[buf].pct;

}

// загрузка и отображение файла
void MainWindow::load_file (QString fl)
{
    buf++;
    pict p;
    p.pct.load(fl);
    toprint.push_back(p);
    show_img (fl);// масшабирование, рисование и подключение сигналов
}
...
void MainWindow::open_pct(QString filename) // добавить картинку в список
{
        ...
        //connect (ldp, SIGNAL(start()), this, SLOT(start_load_picture()));
...
{
    QPagedPaintDevice::Margins mr;
    mr.top=top_m;
    mr.left=left_m;
    mr.right=right_m;
    mr.bottom=bottom_m;
    printer->setMargins(mr);
}
